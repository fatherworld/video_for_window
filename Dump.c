...
#include "C:\\Users\\User\\Desktop\\success\\Dll\\Dll\\ImageHelper\\include\\stdio_fl.h"
...
#include "./ImageHelper/include/Log.h"
#include "./ImageHelper/include/ErrorTools.h"
...
#define FUNC_CODE 0x04
    ...
    if (NULL != file) {
        ...
        vfprintf_fl((FILE*)file, format, _ArgList);
        va_end(_ArgList);
    }
}
...
static int dump_bmp_float(char* filename, char* buf, int width, int height, int channels){
...
static int dump_bmp_uChar(char* filename, char* buf, int width, int height, int channels){
...
#define FUNC_CODE 0x08
	...
	switch (channels){
        case 1:
	        ...
	        break;
        case 2:
            for(i = 0,k=0; i < height * width * 2; i += 2,k+=3){
                
                unsigned char tR,tG,tB;
                tR=tG=tB=0;
//                 printf("原值为 %d",UcharData[i+0]);
//                 printf("右移原值为 %d",UcharData[i+0]>>3);
                tR |=(UcharData[i+0]>>3);
                tG =((UcharData[i+0])& 00000111);
                tG=tG<<3;
                tG |=(UcharData[i+1]>>5);
                tB = (UcharData[i+1] & 00011111);
                mImage.data[k] = tR;
                mImage.data[k + 1] = tG;
                mImage.data[k + 2] = tB;
            }
